...
import { Mail, Phone, MapPin, Send, MessageSquare, Bot, Sparkles, User as UserIcon, BookOpen, SendHorizontal, RefreshCw, CheckCircle2, HelpCircle, Trash2 } from 'lucide-react';
...
export default function Contact({ currentUser }: ContactProps) {
  ...
  // AI Tutor Chat State with local persistence
    ...
    return [
      {
        id: '1',
        sender: 'ai',
        text: `Olá ${currentUser ? currentUser.name.split(' ')[0] : 'estudante'}! Sou o **Professor IA CETEP**, seu assistente pedagógico virtual no estilo ChatGPT.\n\nComo posso te ajudar nos seus estudos hoje? Pode me enviar qualquer dúvida, cálculo ou pedir explicações passo a passo!`,
    ...
    const initial: ChatMessage[] = [
      {
        ...
        sender: 'ai',
        text: `Olá ${currentUser ? currentUser.name.split(' ')[0] : 'estudante'}! Conversa reiniciada. Em que posso te ajudar agora?`,
        timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })
      }
    ];
    setMessages(initial);
