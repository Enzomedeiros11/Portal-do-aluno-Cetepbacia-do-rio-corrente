...
export default function Teachers({ allUsers, onUpdateUsers, currentUser, onRefresh }: TeachersProps) {
  ...
  const handleExportCSV = () => {
    ...
    // Standard semicolon delimiter for Excel in Portuguese/Brazil + UTF-8 BOM (\uFEFF)
    const headers = [
      'ID',
      'Nome Completo',
      'E-mail',
    ...
    const rows = filteredUsers.map(u => {
      const freq = u.frequencia ?? 100;
      const status = freq >= 75 ? 'Frequência Regular' : 'Abaixo do Limite (Risco)';
      return [
        escapeCsv(u.id),
        escapeCsv(u.name || 'Sem nome'),
        escapeCsv(u.email || ''),
        escapeCsv(u.role === 'teacher' ? 'Professor / Gestão' : 'Estudante'),
        escapeCsv(u.course || 'Ensino Técnico Regular'),
        escapeCsv(u.grade || '1º Ano'),
        escapeCsv(`${freq}%`),
        escapeCsv(status),
        escapeCsv(todayStr)
      ].join(';');
    ...
    // Add UTF-8 BOM \uFEFF so Excel automatically parses special characters like ç, ã, é, etc.
    const csvContent = '\uFEFF' + [headers.join(';'), ...rows].join('\r\n');
    const blob = new Blob([csvContent], { type: 'text/csv;charset=utf-8;' });
    const url = URL.createObjectURL(blob);
    const link = document.createElement('a');
    link.href = url;
    link.setAttribute('download', `relatorio_secretaria_cetep_${new Date().toISOString().slice(0, 10)}.csv`);
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    URL.revokeObjectURL(url);
    toast.success('Relatório formatado para Excel exportado com sucesso!');
  };
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 pt-24 pb-12 px-6 print:bg-white print:p-0 transition-colors duration-200">
      <div className="max-w-7xl mx-auto">
        <header className="mb-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-6 print:hidden">
           <div>
              <div className="flex items-center gap-3">
                 <div className="w-10 h-10 bg-slate-900 dark:bg-blue-600 rounded-lg flex items-center justify-center text-white shadow-sm">
                    <Users className="w-6 h-6" />
                 </div>
                 <h1 className="text-3xl font-bold text-slate-900 dark:text-white tracking-tight">Secretaria & Controle de Pessoas</h1>
              </div>
              <p className="text-slate-500 dark:text-slate-400 mt-1">Gerenciamento completo de alunos, professores, notas e frequências.</p>
           </div>

           <div className="flex items-center gap-3 flex-wrap">
              <button 
                onClick={() => setIsAddModalOpen(true)}
                className="flex items-center gap-2 px-4 py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-sm font-semibold transition-colors shadow-sm cursor-pointer"
              >
              ...
              <button 
                ...
                className="flex items-center gap-2 px-4 py-2.5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg text-sm font-semibold text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors shadow-sm cursor-pointer"
                title="Exportar arquivo CSV formatado para abrir no Excel"
              >
                 <Download className="w-4 h-4 text-emerald-600 dark:text-emerald-400" /> Exportar CSV (Excel)
              </button>
              ...
              <button 
                ...
                className="flex items-center gap-2 px-4 py-2.5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg text-sm font-semibold text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors shadow-sm cursor-pointer"
              >
                 <Printer className="w-4 h-4 text-slate-600 dark:text-slate-400" /> Imprimir
              </button>

              <button 
                onClick={async () => { await onRefresh(); toast.success('Dados atualizados!'); }} 
                className="flex items-center gap-2 px-3 py-2.5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg text-sm font-semibold text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors shadow-sm cursor-pointer"
                title="Atualizar"
              >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8 print:hidden">
           ...
           <div className="bg-white dark:bg-slate-900 p-6 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm flex items-center justify-between transition-colors">
              ...
              <div className="w-10 h-10 bg-blue-50 dark:bg-blue-950/60 rounded-lg flex items-center justify-center text-blue-600 dark:text-blue-400">
           ...
           <div className="bg-white dark:bg-slate-900 p-6 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm flex items-center justify-between transition-colors">
              ...
              <div className="w-10 h-10 bg-emerald-50 dark:bg-emerald-950/60 rounded-lg flex items-center justify-center text-emerald-600 dark:text-emerald-400">
           ...
           <div className="bg-white dark:bg-slate-900 p-6 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm flex items-center justify-between transition-colors">
              ...
              <div className="w-10 h-10 bg-amber-50 dark:bg-amber-950/60 rounded-lg flex items-center justify-center text-amber-600 dark:text-amber-400">
        ...
        <div className="bg-white dark:bg-slate-900 p-6 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm mb-6 print:hidden transition-colors">
           <div className="grid grid-cols-1 md:grid-cols-6 gap-4">
              <div className="relative md:col-span-2">
                 ...
                 <input 
                    ...
                    className="w-full pl-10 pr-4 py-2.5 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-sm font-medium text-slate-900 dark:text-white outline-none focus:ring-2 focus:ring-blue-500/20"
                 />
              </div>
              ...
              <select 
                 ...
                 className="px-4 py-2.5 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-sm font-semibold text-slate-800 dark:text-slate-200 outline-none appearance-none"
              >
              ...
              <button 
                ...
                disabled={loading}
                className="bg-slate-900 dark:bg-blue-600 hover:bg-black dark:hover:bg-blue-700 text-white rounded-lg font-bold text-sm transition-colors flex items-center justify-center gap-2 shadow-sm cursor-pointer"
              >
        ...
        <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden transition-colors">
           ...
           <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse">
                 <thead>
                    <tr className="bg-slate-50 dark:bg-slate-800/90 border-b border-slate-200 dark:border-slate-800 text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest">
                          ...
                          <td className="px-4 py-4 text-center">
                             {s.role === 'student' ? (
                               <input 
                                  ...
                                  min="0"
                                  max="100"
                                  className="w-16 h-9 bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded text-center text-sm font-bold text-slate-800 dark:text-white outline-none focus:bg-white dark:focus:bg-slate-700 focus:border-blue-500"
                            ...
                            <td key={field} className="px-4 py-4 text-center">
                               {s.role === 'student' ? (
                                 <input 
                                    type="text"
                                    placeholder="—"
                                    className="w-12 h-9 bg-blue-50/50 dark:bg-blue-950/30 border border-blue-100 dark:border-blue-900/50 rounded text-center text-sm font-bold text-blue-900 dark:text-blue-200 outline-none focus:bg-white dark:focus:bg-slate-800 focus:border-blue-500"
                          ...
                          <td className="px-6 py-4 text-right print:hidden">
                             <div className="flex items-center justify-end gap-1">
                                ...
                                <button 
                                  ...
                                  className="p-2 text-slate-400 hover:text-rose-600 dark:hover:text-rose-400 hover:bg-rose-50 dark:hover:bg-rose-950/40 rounded-lg transition-all cursor-pointer" 
        ...
        <div className="mt-8 grid grid-cols-1 lg:grid-cols-3 gap-6 print:hidden">
           <div className="lg:col-span-2 bg-white dark:bg-slate-900 p-8 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm transition-colors">
              ...
              <div className="space-y-4">
                 <input 
                    ...
                    className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-sm font-bold text-slate-900 dark:text-white outline-none focus:bg-white dark:focus:bg-slate-700 focus:ring-2 focus:ring-blue-500/20"
                 ...
                 <textarea 
                    ...
                    className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-sm font-semibold text-slate-900 dark:text-white outline-none resize-none focus:bg-white dark:focus:bg-slate-700 focus:ring-2 focus:ring-blue-500/20"
                    rows={4}
                 ...
                 <button 
                    ...
                    className="w-full py-3 bg-slate-900 dark:bg-blue-600 text-white rounded-lg font-bold text-sm hover:bg-black dark:hover:bg-blue-700 transition-all flex items-center justify-center gap-2 disabled:opacity-50 cursor-pointer shadow-sm"
                 >
                   ...
                   <div className="mt-6 pt-6 border-t border-slate-200 dark:border-slate-800 space-y-3">
                     ...
                     <div className="space-y-2 max-h-60 overflow-y-auto pr-1">
                       {comunicadosList.map((com) => (
                         <div key={com.id} className="p-3 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl flex items-start justify-between gap-3">
                           ...
                           <button
                             ...
                             className="p-1.5 text-slate-400 hover:text-rose-600 dark:hover:text-rose-400 hover:bg-rose-50 dark:hover:bg-rose-950/40 rounded-lg transition-all shrink-0 cursor-pointer"
           ...
           <div className="bg-slate-900 p-8 rounded-xl text-white shadow-xl shadow-slate-900/10 flex flex-col justify-between border border-slate-800">
              <div>
                ...
                <p className="text-sm text-slate-400 leading-relaxed font-medium">
                   Toda inclusão, alteração ou exclusão de usuários, notas e frequências é sincronizada em tempo real no banco de dados Firebase do CETEP.
                </p>
              </div>
        ...
        {isAddModalOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
            <motion.div 
              ...
              className="bg-white dark:bg-slate-900 rounded-2xl shadow-xl border border-slate-200 dark:border-slate-800 max-w-lg w-full overflow-hidden transition-colors"
            >
              <div className="p-6 bg-slate-900 dark:bg-slate-800 text-white flex items-center justify-between border-b border-slate-800">
              ...
              <form onSubmit={handleCreateUser} className="p-6 space-y-4">
                <div>
                  ...
                  <input 
                    ...
                    className="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-sm font-semibold text-slate-900 dark:text-white outline-none focus:bg-white dark:focus:bg-slate-700 focus:border-blue-500"
                  />
                </div>

                <div>
                  <label className="block text-xs font-bold uppercase text-slate-500 dark:text-slate-400 mb-1">E-mail Corporativo / Pessoal</label>
                  <input 
                    type="email"
                    required
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <select 
                      ...
                      className="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-sm font-semibold text-slate-900 dark:text-white outline-none"
                    >
                  ...
                  <div>
                    ...
                    <input 
                      ...
                      className="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-sm font-semibold text-slate-900 dark:text-white outline-none focus:bg-white dark:focus:bg-slate-700"
                    />
                  </div>
                </div>
                ...
                <div className="pt-4 border-t border-slate-100 dark:border-slate-800 flex items-center justify-end gap-3">
                  <button 
                    ...
                    className="px-4 py-2.5 text-slate-600 dark:text-slate-300 font-bold text-sm hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg transition-colors cursor-pointer"
                  >
                    Cancelar
                  </button>
                  <button 
                    type="submit" 
                    disabled={loading}
                    className="px-6 py-2.5 bg-blue-600 text-white font-bold text-sm rounded-lg hover:bg-blue-700 transition-colors shadow-sm disabled:opacity-50 cursor-pointer"
                  >
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
            ...
            >
              ...
              <form onSubmit={handleSaveUserEdit} className="p-6 space-y-4">
                ...
                <div>
                  <label className="block text-xs font-bold uppercase text-slate-500 dark:text-slate-400 mb-1">E-mail</label>
                  <input 
                    type="email"
                    required
                ...
                <div className="pt-4 border-t border-slate-100 dark:border-slate-800 flex items-center justify-end gap-3">
                  ...
                  <button 
                    type="submit" 
                    disabled={loading}
                    className="px-6 py-2.5 bg-slate-900 dark:bg-blue-600 text-white font-bold text-sm rounded-lg hover:bg-black dark:hover:bg-blue-700 transition-colors shadow-sm disabled:opacity-50 cursor-pointer"
                  >
        ...
        {userToDelete && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
            <motion.div 
              ...
              animate={{ opacity: 1, scale: 1 }}
              exit={{ opacity: 0, scale: 0.95 }}
              className="bg-white dark:bg-slate-900 rounded-2xl shadow-xl border border-slate-200 dark:border-slate-800 max-w-md w-full p-6 text-center transition-colors"
            >
              ...
              <div className="flex items-center justify-center gap-3">
                <button 
                  ...
                  className="px-5 py-2.5 border border-slate-200 dark:border-slate-700 rounded-lg text-slate-700 dark:text-slate-300 font-bold text-sm hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors cursor-pointer"
                >
                  Cancelar
                </button>
                <button 
                  ...
                  disabled={loading}
                  className="px-5 py-2.5 bg-rose-600 text-white rounded-lg font-bold text-sm hover:bg-rose-700 transition-colors shadow-sm disabled:opacity-50 cursor-pointer"
                >
